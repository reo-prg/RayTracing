...
struct Sphere_b {
	float radius;//半径
	Position3 pos; //中心座標
	Sphere_b() :radius(0), pos(0, 0,0) {}
	Sphere_b(float r, Position3& p) :radius(r), pos(p) {}
};
...
struct Plane_b
{
	...
	Plane_b():normal(0.0f, 0.0f, 0.0f), offset(0.0f) {};
	Plane_b(const Vector3& normVec, float off):normal(normVec), offset(off) {};
};
